...
Điền vào `.env`:
- `TAVILY_API_KEY` — lấy tại https://tavily.com (free tier), dùng cho web search — external duy nhất không có cách thay bằng local
- `NVIDIA_API_KEY` — lấy tại https://build.nvidia.com, dùng cho LLM chat (provider mặc định)
- `GROQ_API_KEY` / `OPENAI_API_KEY` — tuỳ chọn, chỉ cần khi đổi `LLM_PROVIDER` sang `groq`/`openai` (xem `app/core/llm.py`)

Không cần key nào nếu đổi `LLM_PROVIDER=ollama` (chạy local hoàn toàn, chậm hơn nhưng không giới hạn số lần gọi) hoặc cho embeddings (luôn chạy local qua Ollama, không phụ thuộc provider chat).

## RAG — build knowledge index

Cài model embedding local (chỉ cần 1 lần):

```bash
ollama pull nomic-embed-text
```

Đặt tài liệu (`.pdf` hoặc `.md`) vào `data/knowledge/`, sau đó build lại index (chạy lại mỗi khi thêm/sửa tài liệu, hoặc khi đổi embedding model):

```bash
python -m app.rag.vectorstore
```

Lệnh này đọc toàn bộ `.pdf`/`.md` trong `data/knowledge/`, chunk, embed (Ollama `nomic-embed-text`), và lưu vào Qdrant local mode tại `data/qdrant_local/`.
...
Phần lớn test (schema, guard, routing, HITL/checkpoint, tool thuần Python) chạy được mà không cần key nào. Vài test full-flow cần `TAVILY_API_KEY` thật, tự động skip nếu thiếu.
